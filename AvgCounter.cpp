 ...
 */
void AvgCounter::InitCounter(Mode mode)
{
 ...
 * as encoders.
 */
AvgCounter::AvgCounter(DigitalSource *source) :
	m_upSource(NULL),
	m_downSource(NULL),
	m_counter(NULL)
{
 ...
 */
AvgCounter::AvgCounter(UINT32 channel) :
	m_upSource(NULL),
	m_downSource(NULL),
	m_counter(NULL)
{
 ...
 */
AvgCounter::AvgCounter(UINT8 moduleNumber, UINT32 channel) :
	m_upSource(NULL),
	m_downSource(NULL),
	m_counter(NULL)
{
 ...
 */
AvgCounter::AvgCounter(AnalogTrigger *trigger) :
	m_upSource(NULL),
	m_downSource(NULL),
	m_counter(NULL)
{
...
AvgCounter::AvgCounter(EncodingType encodingType, DigitalSource *upSource, DigitalSource *downSource, bool inverted) :
	m_upSource(NULL),
	m_downSource(NULL),
	m_counter(NULL)
{
 ...
 */
void AvgCounter::SetUpSource(UINT8 moduleNumber, UINT32 channel)
{
 ...
 */
void AvgCounter::SetUpSource(UINT32 channel)
{
	if (StatusIsFatal()) return;
 ...
 */
void AvgCounter::SetUpSource(AnalogTrigger *analogTrigger, AnalogTriggerOutput::Type triggerType)
{
	if (StatusIsFatal()) return;
 ...
 */
void AvgCounter::SetUpSource(DigitalSource *source)
{
	if (StatusIsFatal()) return;
	if (m_allocatedUpSource)
	{
 ...
 */
void AvgCounter::SetUpSourceEdge(bool risingEdge, bool fallingEdge)
{
	if (StatusIsFatal()) return;
	if (m_upSource == NULL)
	{
 ...
 */
void AvgCounter::SetDownSource(UINT32 channel)
{
	if (StatusIsFatal()) return;
 ...
 * @param channel The digital channel (1..14).
 */
void AvgCounter::SetDownSource(UINT8 moduleNumber, UINT32 channel)
{
	if (StatusIsFatal()) return;
 ...
 * @param triggerType The analog trigger output that will trigger the counter.
 */
void AvgCounter::SetDownSource(AnalogTrigger *analogTrigger, AnalogTriggerOutput::Type triggerType)
{
	if (StatusIsFatal()) return;
 ...
 */
void AvgCounter::SetDownSource(DigitalSource *source)
{
	if (StatusIsFatal()) return;
	if (m_allocatedDownSource)
	{
 ...
 */
void AvgCounter::SetDownSourceEdge(bool risingEdge, bool fallingEdge)
{
	if (StatusIsFatal()) return;
 ...
 */
void AvgCounter::ClearDownSource()
{
	if (StatusIsFatal()) return;
	if (m_allocatedDownSource)
	{
 ...
 */
void AvgCounter::SetUpDownCounterMode()
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;
 ...
 */
void AvgCounter::SetExternalDirectionMode()
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;
 ...
 */
void AvgCounter::SetSemiPeriodMode(bool highSemiPeriod)
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;
 ...
 */
void AvgCounter::SetPulseLengthMode(float threshold)
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;
 ...
 * seconds.
 */
void AvgCounter::SetMaxPeriod(double maxPeriod)
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;
 ...
 */
void AvgCounter::SetUpdateWhenEmpty(bool enabled)
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;
 ...
 */
void AvgCounter::SetReverseDirection(bool reverseDirection)
{
	if (StatusIsFatal()) return;
	tRioStatusCode localStatus = NiFpga_Status_Success;
...
void AvgCounter::UpdateTable() {
	if (m_table != NULL) {
		m_table->PutNumber("Value", Get());
	}
}

void AvgCounter::StartLiveWindowMode() {
	
}

void AvgCounter::StopLiveWindowMode() {
	
}

std::string AvgCounter::GetSmartDashboardType() {
	return "Counter";
}

void AvgCounter::InitTable(ITable *subTable) {
	m_table = subTable;
	UpdateTable();
}

ITable * AvgCounter::GetTable() {
	return m_table;
}


// FPGA additions: (stuff we added)
void AvgCounter::SetSamplesToAverage(int samplesToAverage)
{
	if(samplesToAverage > 127) //127 is not a magic number. 127 came from CD which is apparently a reliable source for (non)magic numbers.
	{
		printf("Too many samples to average!\n");
	}
	tRioStatusCode localStatus = NiFpga_Status_Success;
...
int AvgCounter::GetSamplesToAverage()
{
	tRioStatusCode localStatus = NiFpga_Status_Success;
